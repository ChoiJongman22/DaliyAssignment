...
var Mangu2 = /** @class */ (function () {
    function Mangu2() {
        this.playerList = [];
        this.fightList = [];
        this.squad = [];
    }
    Mangu2.prototype.getRandomElement = function (list) {
        var randomIdx = Math.floor(Math.random() * list.length);
        return list[randomIdx];
    };
    Mangu2.prototype.makeManguTeam = function (number, name, fightCnt, stats, tolerance) {
    ...
    Mangu2.prototype.fight = function () {
    ...
    Mangu2.prototype.reconcile = function () {
        var from = (this.getRandomElement(this.fightList)).from;
        var to = (this.getRandomElement(this.fightList)).to;
        var fidx = this.playerList.findIndex(function (player) { return player.name === from; });
        var tidx = this.playerList.findIndex(function (player) { return player.name === to; });
        if (fidx !== -1 && tidx !== -1) {
            if (this.playerList[tidx].tolerance >= 4) {
                console.log("화해하자 그래!");
                this.playerList[fidx].tolerance += 1;
                this.playerList[tidx].tolerance += 1;
                this.playerList[fidx].stats += 1;
                ...
                this.playerList.splice(fidx, 1);
                var idx = this.fightList.findIndex(function (player) { return player.from === from; });
                var idx2 = this.fightList.findIndex(function (player) { return player.to === to; });
                this.fightList.splice(idx, 1);
                this.fightList.splice(idx2, 1);
                this.playerList.splice(tidx, 1);
                console.log(from, "와", to, ", 둘 다 방출이야 나가");
            }
        }
    };
    Mangu2.prototype.constructionTeam = function () {
